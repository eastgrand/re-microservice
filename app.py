...
try:
    ...
    logging.info("--- Forcing regeneration of cleaned_data.csv from master schema ---")
    load_and_preprocess_data()
    logging.info("--- Data regeneration complete. ---")
    ...
    logging.info("Successfully loaded fresh cleaned_data.csv.")
    logging.info(f"Available columns for analysis: {', '.join(sorted(list(AVAILABLE_COLUMNS)))}")
except Exception as e:
    logging.error(f"FATAL: Could not load or process data on startup: {e}")
